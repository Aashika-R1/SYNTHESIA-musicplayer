...
const ResetPassword = () => {
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      setStatus("success");
      setMessage("password reset sucessfully redirecting");
      setTimeout(() => navigate("/"), 2000);
    } catch (err) {
      setStatus("error");
      setMessage(
        err?.response?.data?.message || "Reset password failed try again",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="reset-wrapper">
      ...
      <p className="reset-subtitle">Enter your password to regain acess</p>
